<?php
	define('__ROOT__', dirname(dirname(__FILE__)));

	require_once(__ROOT__ . '/includes/Login.php');
	require_once(__ROOT__ . '/includes/mbApi.php');
	
	require_once(__ROOT__ . '/includes/classService.php');
	require_once(__ROOT__ . '/includes/clientService.php');
		
/*	require_once(__ROOT__ . '/includes/appointmentService.php');
	require_once(__ROOT__ . '/includes/finderService.php');
	require_once(__ROOT__ . '/includes/saleService.php');
	require_once(__ROOT__ . '/includes/siteService.php');
	require_once(__ROOT__ . '/includes/Belt_List.php');*/
?>